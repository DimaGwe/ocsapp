...
<head>
  ...
  <title><?= $fr ? 'Connexion - Entreprise Centrale - OCSAPP' : 'Sign in - Business Central - OCSAPP' ?></title>
  <?= csrfMeta() ?>
...
<main class="auth-page">
  <section class="auth-brand-panel" aria-label="<?= $fr ? 'Entreprise Centrale' : 'Business Central' ?>">
    <div class="auth-brand-content">
      <div class="auth-eyebrow"><?= $fr ? 'ENTREPRISE CENTRALE' : 'BUSINESS CENTRAL' ?></div>
      ...
      <p><?= $fr
        ? 'Accédez à votre espace entreprise pour l’approvisionnement, la distribution et la gestion de vos activités.'
        : 'Access your business workspace for procurement, distribution and operations management.' ?></p>
  ...
  <section class="auth-form-panel" id="main-content">
    <div class="auth-card">
      ...
      <p class="subtitle"><?= $fr
        ? 'Connectez-vous avec les identifiants associés à votre compte Entreprise Centrale.'
        : 'Sign in with the credentials associated with your Business Central account.' ?></p>
      ...
      <form action="<?= url('distribution/login') ?>" method="POST">
        <?= csrfField() ?>
        <div class="form-group">
          <label class="form-label" for="email"><?= $fr ? 'Adresse courriel' : 'Email address' ?></label>
          <div class="input-wrapper">
            <i class="fa-solid fa-envelope input-icon"></i>
            <input autocomplete="email" autofocus class="form-input" id="email" name="email" placeholder="business@company.com" required type="email" value="<?= htmlspecialchars(old('email') ?? '') ?>">
          </div>
        </div>
        <div class="form-group">
        ...
        <div class="form-options">
          <div class="checkbox-wrapper">
            ...
            <label class="checkbox-label form-label" for="remember"><?= $fr ? 'Se souvenir de moi' : 'Remember me' ?></label>
          </div>
          <a class="forgot-link" href="<?= url('forgot-password?portal=distribution') ?>"><?= $fr ? 'Mot de passe oublié?' : 'Forgot password?' ?></a>
        </div>
        <button class="btn-submit ocs-submit" type="submit"><i class="fa-solid fa-right-to-bracket"></i> <?= $fr ? 'Se connecter' : 'Sign in' ?></button>
      </form>
      <div class="register-block"><?= $fr ? 'Vous n’avez pas encore accès?' : "Don't have access yet?" ?> <a href="<?= url('distribution/register') ?>"><?= $fr ? 'Créer un compte entreprise' : 'Create a business account' ?></a></div>
      <div class="auth-help"><a href="<?= url('forgot-password?portal=distribution') ?>"><?= $fr ? 'Mot de passe oublié?' : 'Forgot password?' ?></a><a href="<?= url('contact') ?>"><?= $fr ? 'Besoin d’aide? Contactez OCSAPP' : 'Need help? Contact OCSAPP' ?></a></div>
