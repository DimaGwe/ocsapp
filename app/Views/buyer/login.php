...
<head>
  ...
  <title><?= $fr ? 'Connexion - Acheteur Central - OCSAPP' : 'Sign in - Buyer Central - OCSAPP' ?></title>
  <?= csrfMeta() ?>
...
<header class="auth-topbar">
  <a class="auth-logo" href="<?= url('/') ?>" aria-label="<?= $fr ? 'OCSAPP - Accueil' : 'OCSAPP' ?>">OCSAPP</a>
  <nav class="auth-toplinks" aria-label="<?= $fr ? 'Navigation de connexion' : 'Login navigation' ?>">
    ...
    <a class="central-link" href="<?= url('buyer-central') ?>"><?= $fr ? 'Acheteur Central' : 'Buyer Central' ?></a>
  </nav>
</header>
<main class="auth-page">
  <section class="auth-brand-panel" aria-label="<?= $fr ? 'Acheteur Central' : 'Buyer Central' ?>">
    <div class="auth-brand-content">
      <div class="auth-eyebrow"><?= $fr ? 'ACHETEUR CENTRAL' : 'BUYER CENTRAL' ?></div>
      <div class="central-icon"><img src="<?= asset('images/about/central-buyer.png') ?>" alt="<?= $fr ? 'Acheteur Central' : 'Buyer Central' ?>"></div>
      <h1><?= $fr ? 'Acheteur Central' : 'Buyer Central' ?></h1>
      <p><?= $fr
        ? 'Accédez à votre compte acheteur et à vos activités dans l’écosystème OCSAPP.'
        : 'Access your buyer account and your activity across the OCSAPP ecosystem.' ?></p>
  ...
  <section class="auth-form-panel" id="main-content">
    <div class="auth-card">
      ...
      <p class="subtitle"><?= $fr
        ? 'Connectez-vous avec les identifiants associés à votre compte Acheteur Central.'
        : 'Sign in with the credentials associated with your Buyer Central account.' ?></p>
      ...
      <form action="<?= url('login') ?>" method="POST">
        ...
        <div class="form-group">
          ...
          <div class="input-wrapper">
            ...
            <input autocomplete="email" autofocus class="form-input" id="email" name="email" placeholder="you@example.com" required type="email" value="<?= htmlspecialchars(old('email') ?? '') ?>">
          </div>
        </div>
        <div class="form-group">
      ...
      <div class="register-block"><?= $fr ? 'Vous n’avez pas encore accès?' : "Don't have access yet?" ?> <a href="<?= url('register') ?>"><?= $fr ? 'Créer un compte' : 'Create an account' ?></a></div>
