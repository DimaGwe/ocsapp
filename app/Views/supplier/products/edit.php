<?php
$pageTitle = (($_SESSION['language'] ?? 'fr') === 'fr') ? 'Modifier le produit' : 'Edit Product';
...
<style>
  ...
  .new-preview.active { display: block; }
  ...
  .remove-image-btn.active { display: inline-flex; }
...
<div class="form-card">
  <div class="form-header">
    <h2><?= $fr ? 'Modifier le produit' : 'Edit Product' ?></h2>
    <p><?= $fr ? 'Mettez à jour les informations de votre produit' : 'Update your product information' ?></p>
  </div>

  <form method="POST" action="<?= url('supplier/products/update') ?>" enctype="multipart/form-data">
    <?= csrfField() ?>
    ...
    <div class="form-group">
      ...
      <!-- Hidden file input -->
      <input type="file" id="image" name="image" accept="image/*" style="display:none" onchange="previewNewImage(this)">
      ...
      <button type="button" class="remove-image-btn" id="removeNewBtn" onclick="removeNewImage()">
        <i class="fas fa-times"></i> <?= $fr ? 'Supprimer la nouvelle image' : 'Remove New Image' ?>
      </button>
    </div>
    ...
    <div class="form-group">
      ...
      <select id="marketplace_product_id" name="marketplace_product_id" class="form-select">
        ...
        <?php if (!empty($marketplaceProducts)): ?>
          <?php foreach ($marketplaceProducts as $mp): ?>
            <option value="<?= $mp['id'] ?>" <?= ($product['marketplace_product_id'] ?? '') == $mp['id'] ? 'selected' : '' ?>>
      ...
      <p class="form-hint"><?= $fr ? 'Liez ce produit à un article de la marketplace pour activer les mises à jour automatiques du stock' : 'Link this to a product in the marketplace to enable automatic stock updates when purchase orders are received' ?></p>
    </div>
    ...
    <div class="form-grid">
      ...
      <div class="form-group">
        ...
        <select id="unit" name="unit" class="form-select">
          <option value="unit" <?= ($product['unit'] ?? 'unit') === 'unit' ? 'selected' : '' ?>><?= $fr ? 'Unité' : 'Unit' ?></option>
          <option value="box" <?= ($product['unit'] ?? '') === 'box' ? 'selected' : '' ?>><?= $fr ? 'Boîte' : 'Box' ?></option>
          <option value="case" <?= ($product['unit'] ?? '') === 'case' ? 'selected' : '' ?>><?= $fr ? 'Caisse' : 'Case' ?></option>
          <option value="kg" <?= ($product['unit'] ?? '') === 'kg' ? 'selected' : '' ?>><?= $fr ? 'Kilogramme (kg)' : 'Kilogram (kg)' ?></option>
          <option value="lb" <?= ($product['unit'] ?? '') === 'lb' ? 'selected' : '' ?>><?= $fr ? 'Livre (lb)' : 'Pound (lb)' ?></option>
          <option value="liter" <?= ($product['unit'] ?? '') === 'liter' ? 'selected' : '' ?>><?= $fr ? 'Litre' : 'Liter' ?></option>
          <option value="gallon" <?= ($product['unit'] ?? '') === 'gallon' ? 'selected' : '' ?>>Gallon</option>
          <option value="dozen" <?= ($product['unit'] ?? '') === 'dozen' ? 'selected' : '' ?>><?= $fr ? 'Douzaine' : 'Dozen' ?></option>
        </select>
      </div>
    </div>
    ...
    <div class="form-grid">
      <div class="form-group">
        ...
        <input
          ...
          class="form-input"
          min="0"
          value="<?= $product['lead_time_days'] ?? 7 ?>"
        >
    ...
    <div class="form-group">
      ...
      <textarea
        ...
        rows="3"
        class="form-textarea"
      ><?= htmlspecialchars($product['notes'] ?? '') ?></textarea>
    </div>
    ...
    <div class="form-actions">
      ...
      <button type="submit" class="btn btn-primary">
        <i class="fas fa-save"></i> <?= $fr ? 'Mettre à jour le produit' : 'Update Product' ?>
      </button>
    </div>
  </form>
</div>
