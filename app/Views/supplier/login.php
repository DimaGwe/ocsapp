...
<head>
  ...
  <title><?= $fr ? 'Connexion - Fournisseur Central - OCSAPP' : 'Sign in - Supplier Central - OCSAPP' ?></title>
  <?= csrfMeta() ?>
...
<main class="auth-page">
  <section class="auth-brand-panel" aria-label="<?= $fr ? 'Fournisseur Central' : 'Supplier Central' ?>">
    <div class="auth-brand-content">
      <div class="auth-eyebrow"><?= $fr ? 'FOURNISSEUR CENTRAL' : 'SUPPLIER CENTRAL' ?></div>
      ...
      <p><?= $fr
        ? 'Accédez à votre espace fournisseur pour gérer vos produits, vos commandes et vos relations commerciales.'
        : 'Access your supplier workspace to manage products, orders and business relationships.' ?></p>
  ...
  <section class="auth-form-panel" id="main-content">
    <div class="auth-card">
      ...
      <p class="subtitle"><?= $fr
        ? 'Connectez-vous avec les identifiants associés à votre compte Fournisseur Central.'
        : 'Sign in with the credentials associated with your Supplier Central account.' ?></p>

      <?php if (hasFlash('error')): ?>
        <div class="alert alert-error"><i class="fa-solid fa-circle-exclamation"></i> <?= getFlash('error') ?></div>
      <?php endif; ?>
      <?php if (hasFlash('success')): ?>
        <div class="alert alert-success"><i class="fa-solid fa-circle-check"></i> <?= getFlash('success') ?></div>
      <?php endif; ?>

      <form action="<?= url('supplier/login') ?>" method="POST">
        <?= csrfField() ?>
        <div class="form-group">
          <label class="form-label" for="email"><?= $fr ? 'Adresse courriel' : 'Email address' ?></label>
          <div class="input-wrapper">
            <i class="fa-solid fa-envelope input-icon"></i>
            <input autocomplete="email" autofocus class="form-input" id="email" name="email" placeholder="fournisseur@entreprise.com" required type="email" value="<?= htmlspecialchars(old('email') ?? '') ?>">
          </div>
        </div>
        <div class="form-group">
          ...
          <div class="password-wrapper">
            ...
            <input autocomplete="current-password" class="form-input" id="password" name="password" placeholder="<?= $fr ? 'Entrez votre mot de passe' : 'Enter your password' ?>" required type="password">
        ...
        <div class="form-options">
          <div class="checkbox-wrapper">
            ...
            <label class="checkbox-label form-label" for="remember"><?= $fr ? 'Se souvenir de moi pendant 30 jours' : 'Remember me for 30 days' ?></label>
          </div>
          <a class="forgot-link" href="<?= url('supplier/forgot-password') ?>"><?= $fr ? 'Mot de passe oublié?' : 'Forgot password?' ?></a>
        </div>
        <button class="btn-submit ocs-submit" type="submit"><i class="fa-solid fa-right-to-bracket"></i> <?= $fr ? 'Se connecter' : 'Sign in' ?></button>
      </form>
      <div class="register-block"><?= $fr ? 'Vous n’avez pas encore accès?' : "Don't have access yet?" ?> <a href="<?= url('supplier-central') ?>"><?= $fr ? 'Découvrir Fournisseur Central' : 'Explore Supplier Central' ?></a></div>
      <div class="auth-help"><a href="<?= url('supplier/forgot-password') ?>"><?= $fr ? 'Mot de passe oublié?' : 'Forgot password?' ?></a><a href="<?= url('contact') ?>"><?= $fr ? 'Besoin d’aide? Contactez OCSAPP' : 'Need help? Contact OCSAPP' ?></a></div>
