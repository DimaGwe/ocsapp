<?php
/**
 * Promo Banner - Edit View
 * File: app/Views/admin/promo-banners/edit.php
 */

$pageTitle = $pageTitle ?? 'Edit Promo Banner';
...
<!-- Edit Form -->
<div class="edit-card">
  <form method="POST" action="<?= url('/admin/promo-banners/update') ?>">
    <?= csrfField() ?>
    ...
    <div class="form-section">
      ...
      <textarea id="subtitle"
                name="subtitle"
                class="form-textarea"
                rows="2"><?= htmlspecialchars($banner['subtitle'] ?? '') ?></textarea>
    </div>
    ...
    <div class="form-section">
      ...
      <input type="number"
             ...
             class="form-input"
             value="<?= $banner['discount_percentage'] ?? 20 ?>"
             min="0"
             max="100"
             required>
    </div>
      ...
      <div class="product-select-container" id="productSelectContainer">
        <?php if (!empty($ocsProducts)): ?>
          <?php foreach ($ocsProducts as $product): ?>
            ...
            ?>
            <div class="product-select-item <?= $isSelected ? 'selected' : '' ?>"
    ...
    <div class="form-section">
      ...
      <input type="text"
             ...
             name="button_text"
             class="form-input"
             value="<?= htmlspecialchars($banner['button_text'] ?? 'Shop Now') ?>"
    ...
    <div class="form-section">
      ...
      <input type="text"
             ...
             name="button_url"
             class="form-input"
             value="<?= htmlspecialchars($banner['button_url'] ?? '/deals') ?>"
             placeholder="/deals">
    </div>
    ...
    <div class="form-section">
      ...
      <div style="display: flex; align-items: center; gap: 16px;">
        <label class="toggle-switch">
          <input type="checkbox"
                 id="statusCheckbox"
                 <?= ($banner['status'] ?? 'active') === 'active' ? 'checked' : '' ?>>
          <span class="toggle-slider"></span>
        </label>
        <span id="statusText" style="color: var(--text); font-weight: 500;">
          <?= ($banner['status'] ?? 'active') === 'active' ? 'Active (Visible on website)' : 'Inactive (Hidden)' ?>
        </span>
      </div>
      <input type="hidden" name="status" id="statusInput" value="<?= $banner['status'] ?? 'active' ?>">
    </div>
