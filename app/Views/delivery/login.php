...
<head>
  ...
  <title><?= $fr ? 'Connexion - Livreur Central · ODA - OCSAPP' : 'Sign in - Driver Central · ODA - OCSAPP' ?></title>
  <?= csrfMeta() ?>
...
<main class="auth-page">
  <section class="auth-brand-panel" aria-label="<?= $fr ? 'Livreur Central · ODA' : 'Driver Central · ODA' ?>">
    <div class="auth-brand-content">
      <div class="auth-eyebrow"><?= $fr ? 'LIVREUR CENTRAL' : 'DRIVER CENTRAL' ?></div>
      ...
      <p><?= $fr
        ? 'Accédez à votre tableau de bord livreur et à vos activités de livraison dans le réseau ODA.'
        : 'Access your driver dashboard and delivery activity across the ODA network.' ?></p>
  ...
  <section class="auth-form-panel" id="main-content">
    <div class="auth-card">
      ...
      <p class="subtitle"><?= $fr
        ? 'Connectez-vous avec les identifiants associés à votre compte Livreur Central · ODA.'
        : 'Sign in with the credentials associated with your Driver Central · ODA account.' ?></p>
      ...
      <form action="<?= url('login') ?>" method="POST">
        ...
        <div class="form-group">
          ...
          <div class="input-wrapper">
            ...
            <input autocomplete="email" autofocus class="form-input" id="email" name="email" placeholder="driver@example.com" required type="email" value="<?= htmlspecialchars(old('email') ?? '') ?>">
          </div>
        </div>
        <div class="form-group">
      ...
      <div class="register-block"><?= $fr ? 'Vous n’avez pas encore accès?' : "Don't have access yet?" ?> <a href="<?= url('delivery/apply') ?>"><?= $fr ? 'Postuler comme livreur' : 'Apply to become a driver' ?></a></div>
