...
$pending = $_SESSION['pending_driver_verification'] ?? [];
$email   = $pending['email'] ?? '';
...
$attempts    = $_SESSION['driver_verification_attempts'] ?? 0;
$maxAttempts = 5;
...
<head>
    ...
    <title><?= $fr ? 'Vérification courriel - OCSAPP Livreur' : 'Email Verification - OCSAPP Driver' ?></title>
    ...
    <style>
        ...
        .verify-hero h1 { font-size: clamp(22px, 4vw, 32px); font-weight: 800; color: white; margin-bottom: 10px; }
        .verify-hero p  { font-size: 15px; color: rgba(255,255,255,0.7); max-width: 440px; margin: 0 auto; line-height: 1.6; }
        ...
        .lock-icon {
            ...
            margin: 0 auto 20px;
            font-size: 30px; color: #00b207;
            border: 2px solid #bbf7d0;
        }
        .verify-card h2  { font-size: 20px; font-weight: 700; color: #1a1a1a; margin-bottom: 10px; }
        .verify-card .sub { font-size: 14px; color: #6b7280; line-height: 1.6; margin-bottom: 28px; }
        ...
        .otp-box:focus  { border-color: #00b207; box-shadow: 0 0 0 3px rgba(0,178,7,0.12); background: #fff; }
        ...
        .attempts-bar { font-size: 12px; color: #9ca3af; margin-bottom: 20px; }
        ...
        .btn-verify .spinner { display: none; width: 18px; height: 18px; border: 2px solid rgba(255,255,255,0.3); border-top-color: white; border-radius: 50%; animation: spin 0.6s linear infinite; }
        ...
        .btn-resend { background: none; border: none; color: #00b207; font-size: 13px; font-weight: 600; cursor: pointer; font-family: inherit; text-decoration: underline; padding: 0; }
...
<div class="verify-hero">
    <div class="verify-hero-badge">
        ...
        <?= $fr ? 'Portail Livreur' : 'Driver Portal' ?>
    </div>
    <h1><?= $fr ? 'Vérification de votre courriel' : 'Verify Your Email' ?></h1>
    <p><?= $fr ? 'Une étape rapide pour confirmer votre candidature.' : 'One quick step to confirm your application.' ?></p>
</div>
...
<div class="verify-page">
    <div class="verify-card">
        ...
        <?php $errorMsg = getFlash('error'); if ($errorMsg): ?>
        ...
        <form method="POST" action="<?= url('delivery/verify-email') ?>" id="verifyForm">
            <?= csrfField() ?>
            ...
            <?php if ($attempts > 0 && $remaining > 0): ?>
                <p class="attempts-bar <?= $remaining <= 2 ? 'low' : '' ?>">
                    <?= $fr ? "Tentatives restantes : {$remaining}" : "Attempts remaining: {$remaining}" ?>
                </p>
            <?php endif; ?>
        ...
        <div class="resend-form">
            ...
            <form method="POST" action="<?= url('delivery/resend-verification') ?>" style="display:inline;">
                <?= csrfField() ?>
...
boxes.forEach((box, i) => {
    ...
    box.addEventListener('keydown', function (e) {
        if (e.key === 'Backspace' && !this.value && i > 0) { boxes[i - 1].value = ''; boxes[i - 1].focus(); syncHidden(); }
