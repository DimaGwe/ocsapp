...
class AdminDeliveryController {
    ...
    public function earnings() {
        ...
        // generate_weekly_payout_batches.php.
        $batches = $this->db->query("
            SELECT * FROM driver_payout_batches ORDER BY batch_date DESC LIMIT 10
        ")->fetchAll(PDO::FETCH_ASSOC);
    ...
    /**
     * Mark an entire weekly driver payout batch as paid in one action - the
     * per-row bulk select above only covers what's rendered on the page
     * (LIMIT 100), this covers every pending row in the batch regardless.
     */
    public function markBatchPaid() {
        AuthMiddleware::checkRouteAccess();
        ...
        try {
            ...
            if (!$batchId) {
                throw new Exception('No batch specified');
            }
            ...
            $this->db->prepare("
                UPDATE driver_payout_batches SET status = 'completed', completed_at = NOW() WHERE id = ?
            ")->execute([$batchId]);
            ...
            return jsonResponse([
                'success' => true,
                'message' => "Batch marked paid - {$count} earning(s) updated."
            ]);
        } catch (Exception $e) {
            if ($this->db->inTransaction()) {
                $this->db->rollBack();
